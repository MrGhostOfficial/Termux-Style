...
decoration() {
	...
	for i in zenburn.properties termux.properties Go.ttf; do
	...
	for i in zenburn.properties termux.properties Go.ttf; do
		git clone https://github.com/MrGhostOfficial/Termux-Style/$i -O ~/.termux/$i
	done
	#rm -r $PREFIX/etc/motd
	#echo "toilet -F metal -F border -f future Termux-Style" >> $PREFIX/etc/bash.bashrc
	cp $PREFIX/etc/bash.bashrc $PREFIX/etc/bash.bashrc.bk
	sed -i s:PS1.*:""neofetch"\nPS1=\'\\\\[\\\\e\[0\;31m\\\\]~ \\\[\\\e[0;32m\\\\]\# \\\[\\\e[0;37m\\\\]\'": $PREFIX/etc/bash.bashrc
...
echo "[i] Installing TermuxStyle ..."
printf "[?] Do you want TermuxStyle Custom PS1? [Y/n] "
read ask
if [[ "$ask" == [Y/y] ]];then
decoration
fi
addrepo
echo "[i] TermuxStyle Installed Successfully."
...
echo "[i] TermuxStyle Repository couldn't found !, Make Install First."
exit 0
fi
...
if [[ -f "$HOME/.termux/configure.bk" ]];then
	...
	echo "[i] TermuxStyle Uninstalled Successfully."
	exit 0
fi
...
command for usage :
bash $(basename $0) -i : for install termux-style
bash $(basename $0) -u : for uninstall termux-style
"
else
echo -e "
command for usage :
